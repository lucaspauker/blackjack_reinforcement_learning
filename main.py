...
def get_timesteps_vs_win_rate_data():
    ...
    for i in range(GRAPH_NUM_TIMESTEPS):
        ...
        win_rate, loss_rate = test_policy(p, env, n=1000000)
...
def graph_timesteps_vs_win_rate(win_loss_rates):
    ...
    plt.plot(range(1, GRAPH_NUM_TIMESTEPS + 1), win_rates, color="green", label="Q learning")
    plt.title("Win rate vs. number of time steps")
    plt.xlabel("Number of time steps")
    plt.ylabel("Win rate")
    plt.hlines(optimal_win_rate, 0, GRAPH_NUM_TIMESTEPS, color="red", label="Basic strategy")
    plt.hlines(stand_win_rate, 0, GRAPH_NUM_TIMESTEPS, color="orange", label="Stand only strategy")
    plt.legend()
...
def test_random_policy():
    ...
    return test_policy(p, env, n=1000000)
...
if __name__ == "__main__":
    ...
    with open("tmp/win_loss_rates.txt", "w") as f:
    ...
    if False:
        ...
        s.step_through()
        s.save_policy(filename="output/Policy.txt")
        s.print_policy()
